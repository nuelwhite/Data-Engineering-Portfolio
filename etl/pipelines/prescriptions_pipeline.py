...
logger = setup_logger('prescriptions_pipeline')

# Pipeline configuration constants
SOURCE_TABLE = 'prescriptions_data'  # Source table in MySQL
TARGET_TABLE = 'prescriptions_data'  # Target table in PostgreSQL
...
def process_batch(batch, transformer):
    """
    Process a batch of prescription records.
    ...
    try:
        ...
        df = transformer.standardize_dates(df, ['dispensed_date'])
        
        # Remove any duplicate records
        df = transformer.remove_duplicates(df)
        
        # Standardize drug names (Title Case)
        df['drug_name'] = df['drug_name'].str.strip().str.title()
        
        # Standardize dosage text
        df['dosage'] = df['dosage'].str.strip().str.capitalize()
        
        # Handle missing values
        df = transformer.handle_missing_values(
            df,
            categorical_columns=['drug_name', 'dosage'],  # Fill with mode
            date_columns=['dispensed_date']  # Fill with current date
        )
...
def run_pipeline():
    """Run the prescriptions data pipeline"""
    try:
        logger.info("Starting prescriptions data pipeline...")
        ...
        try:
            ...
            for batch, last_id in idempotency.process_in_batches(
                mysql_engine,
                SOURCE_TABLE,
                'prescription_id',  # Primary key for tracking progress
                BATCH_SIZE
            ):
                try:
            ...
            logger.info("Prescriptions data pipeline completed successfully")
        ...
        logger.error(f"Error in prescriptions pipeline: {str(e)}")
        raise
