...
logger = setup_logger('inventory_pipeline')

# Pipeline configuration constants
SOURCE_TABLE = 'inventory_items'  # Source table in MySQL
TARGET_TABLE = 'inventory_items'  # Target table in PostgreSQL
...
def process_batch(batch, transformer):
    """
    Process a batch of inventory records.
    ...
    try:
        ...
        # Standardize item names (Title Case)
        ...
        df['reorder_level'] = pd.to_numeric(df['reorder_level'], errors='coerce').fillna(50).astype(int)
        
        # Handle missing values
        df = transformer.handle_missing_values(
            df,
            numeric_columns=['quantity', 'reorder_level'],  # Fill with 0 and 50 respectively
            categorical_columns=['item_name', 'supplier'],  # Fill with mode
...
def run_pipeline():
    """Run the inventory data pipeline"""
    try:
        logger.info("Starting inventory data pipeline...")
        ...
        try:
            ...
            for batch, last_id in idempotency.process_in_batches(
                mysql_engine,
                SOURCE_TABLE,
                'item_id',  # Primary key for tracking progress
                BATCH_SIZE
            ):
                try:
            ...
            logger.info("Inventory data pipeline completed successfully")
